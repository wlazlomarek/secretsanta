...
async function loadPairing(
  ...
  if (
    ...
    return [
      name,
      { name: CryptoJS.AES.decrypt(pairing, key).toString(CryptoJS.enc.Utf8) },
    ];
  }
    ...
    <Layout menuItems={menuItems}>
      <div>
        {!loading && assignment && (
          ...
          >
            <PostCard>
              <h1 className="text-4xl font-bold mb-6 text-center text-red-700 font-dancing-script">
                {t("pairing.title")}
              </h1>
              <p className="mb-6 text-xl text-center text-gray-600">
                <Trans
                  ...
                  components={{
                    name: (
                      <span className="font-semibold">{assignment![0]}</span>
                    ),
                  }}
                />
              </p>
